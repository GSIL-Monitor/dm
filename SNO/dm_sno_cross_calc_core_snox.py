...
#         20160708 黄叶建huangyj modify
    ...
    def divide_by_lat0(self):
        ...
        '''
        y_cross = np.where(
            np.diff((self.orbit['Lat'] < 0.).astype('int')))[0] + 1
        self.divide.extend(y_cross)
        self.divide.sort()
            ...
            for j in xrange(len(s2.divide)):
                ...
                if len(inters) > 0:
                    ...
                    if abs(timeDiff) <= timeGap_low * 60:
                        ...
                        trails1 = [s1.orbit['Lon'][st: et],
                                   s1.orbit['Lat'][st: et]]
                        st = max(index2 - 240, 0)
                        et = min(index2 + 80, len(s2.orbit['Lon']))
                        trails2 = [s2.orbit['Lon'][st: et],
                                   s2.orbit['Lat'][st: et]]
...
def runGEO_LEO(s1, s2, sat_dist, outPath, Log):
    ...
    lat0 = s2.orbit['Lat'][0]
    lon0 = s2.orbit['Lon'][0]
    latlonInfo = [lat0 + sat_dist, lat0 -
                  sat_dist, lon0 - sat_dist, lon0 + sat_dist]
    s1.setArea(latlonInfo)
    if s1.error:
        return
    ...
    for day_num in range(0, day_counts):
        ...
        for i in xrange(len(s1.divide)):
            ...
            for j in xrange(len(s2.divide)):
                ...
                sz = solar_zen(
                    yy, mm, dd, hh, s1.orbit['Lon'][index1], s1.orbit['Lat'][index1])
                if sz > zangle:
                    continue
