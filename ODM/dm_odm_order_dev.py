...
def delete_repeat(timelist, filelist):
    print u'过滤前 %d' % len(timelist)
    FileInfoList = GetFileInfo(filelist)
    new_time_list = []
...
#             print e_ymdhms1
        ...
        if flag == 1:
            ...
            new_time_list.append([s_ymdhms2, e_ymdhms2])
    print u'过滤后 %d' % len(new_time_list)
...
#         return
    ...
    # wangpeng add delete repeat order time
    mvrec_file = os.path.join(MVREC_DIR, satID, '%s.txt' % ymd)
    if os.path.isfile(mvrec_file):
        fp = open(mvrec_file, 'r')
        lines = fp.readlines()
        fp.close()
        new_time_lsit = delete_repeat(timeList, lines)
    else:
        new_time_lsit = timeList

    # 将重叠部分时间进行扩展形成新的时间段（在web订购方式时会需要，避免重复订购）
    if len(new_time_lsit) != 0:
        combine_timeList = CombineTimeList(new_time_lsit)
    else:
...
MVREC_DIR = inCfg['PATH']['OUT']['MVREC']
